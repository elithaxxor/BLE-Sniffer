import bluetooth, subprocess, bluetooth, logging, sqlite3, time, os, sys
import threading
...
def log_device_info(device):
    """
    Logs the device information into the database.
    """
    addr, name = device
...
def scan_devices():
    """
    Performs a Bluetooth scan, retrieves extra device information,
    ...
    try:
        ...
        # Use multiprocessing Pool to concurrently fetch and log device info
        with Pool(cpu_count()) as pool:
            pool.map(log_device_info, devices)
    except Exception as e:
